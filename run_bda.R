...
run_bdgraph_sims <- function(conditions_grid, mcmc_iter = 5000, reps = 25, burnin =  2000, results_df) {
...
for (i in 1:nrow(expanded_grid)) {
  ...
  for (rep in 1:reps) {
    ...
    results_df_rj <- rbind(results_df_rj, current_run_results)
    cat("\n")

    if (i %% 24 == 0) saveRDS(results_df_rj, paste0("out/df_rj_", i))
  }
}
