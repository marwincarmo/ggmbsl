...
g_df |>
  dplyr::mutate(g_prior = factor(g_prior), g_true = factor(g_true),
                n = factor(n)) |>
  dplyr::filter(b_prior == 3,
                b_true == 3,
                #relative_rmse < 6,
                p == 25,) |>
  ggplot(aes(x = n, y = specificity, color = model)) +
  geom_boxplot() +
  facet_grid(g_true ~ g_prior)
...
s_bggm |>
  dplyr::mutate(true_sd_pcor = factor(true_sd_pcor),
                prior_sd_pcor = factor(prior_sd_pcor),
                graph_prob = factor(graph_prob),
                n = factor(n)) |>
  dplyr::filter(p == auc
                ) |>
  ggplot(aes(x = n, y = specificity, color = graph_prob)) +
  geom_boxplot() +
  facet_grid(true_sd_pcor ~ prior_sd_pcor)
...
s_bggm |>
  ...
  ##        true_h==prior_h
  ##        ) |>
  dplyr::select(auc_val, f1_score, p_plus, p_minus, precision, sensitivity, specificity, rmse, relative_rmse) |>
  xtable()
